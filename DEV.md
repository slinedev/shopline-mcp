...
│   ├── tools/
│   │   ├── assistant.ts      # 能力索引、工具搜索、工作流建议、写入预览
│   │   ├── register.ts       # 工具注册
│   │   ├── custom.ts         # Python parity business handlers
│   │   ├── generic.ts        # 通用 endpoint executor
│   │   └── operationPlan.ts  # 请求/预览共用的 endpoint/body 推导
│   └── generated/
...
- 6 个 assistant tools 不属于 Python parity 基线，因此 `SHOPLINE_TOOL_SPECS` 仍只统计 143 个业务工具。
...
9. 运行时参数 `store_alias`、`dry_run`、`confirm_write` 不属于 Shopline API 参数，不能透传到 query 或 body。
